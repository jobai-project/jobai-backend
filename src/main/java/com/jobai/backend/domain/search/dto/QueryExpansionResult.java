 ...
 * 쿼리 확장 결과.
 *
 * <p>unmatched 토큰을 matchType × importance 기준으로 4가지로 분류하며,
 * 각 분류는 개념 그룹 단위로 표현한다.
 *
 * <ul>
 *   <li>exactRequired   — 공고에 반드시 등장해야 하는 기술 스택·도구명 그룹 (SQL AND)</li>
 *   <li>exactPreferred  — 있으면 좋지만 필수 아닌 기술 그룹 (정렬 신호)</li>
 *   <li>semanticRequired  — 반드시 충족해야 하는 의미 조건의 유의어 그룹 (SQL AND + 임베딩)</li>
 *   <li>semanticPreferred — 선호 분위기·문화 표현의 유의어 그룹 (임베딩 + 정렬 신호)</li>
 * </ul>
 *
...
public record QueryExpansionResult(
        ...
        List<RequirementGroup> exactPreferred,
    ...
    public static QueryExpansionResult unchanged(String originalQuery) {
        return new QueryExpansionResult(
                originalQuery, List.of(), List.of(), List.of(), List.of(), List.of());
    }

    /** 쿼리가 확장되었는지 여부를 반환한다. */
    public boolean wasExpanded() {
        return !exactRequired.isEmpty() || !exactPreferred.isEmpty()
