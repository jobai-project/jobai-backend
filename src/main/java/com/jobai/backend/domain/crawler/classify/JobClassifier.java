...
public class JobClassifier {
    ...
    /** 한 배치를 분류. 실패하면 배치 크기만큼 UNCLASSIFIED 로 채워 반환(개수 보존). */
    private List<JobCategory> classifyBatch(List<String> batch) {
        try {
            ...
            if (parsed == null) {
                log.warn("분류 응답 개수 불일치 — 배치 {}건 미분류 처리", batch.size());
                return unclassified(batch.size());
            }
            return parsed;
        } catch (LlmException e) {
            log.warn("분류 배치 실패({}건) — 미분류 처리: {}", batch.size(), e.getMessage());
            return unclassified(batch.size());
        }
    }

    /** 제목 목록을 번호 매겨 유저 메시지로. */
    private String buildUserText(List<String> batch) {
        StringBuilder sb = new StringBuilder("다음 제목들을 분류하세요:\n");
    ...
    private List<JobCategory> unclassified(int n) {
        return new ArrayList<>(Collections.nCopies(n, JobCategory.UNCLASSIFIED));
    }
}
