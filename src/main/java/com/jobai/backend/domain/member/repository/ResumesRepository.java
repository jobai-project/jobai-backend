...
public interface ResumesRepository extends JpaRepository<Resumes, Long> {
    ...
    /** 배치 점수 산출(사기업): 활성이면서 임베딩이 생성된 이력서 전체 조회 (Member + careerTypes JOIN FETCH) */
    @Query("SELECT DISTINCT r FROM Resumes r JOIN FETCH r.member m LEFT JOIN FETCH m.careerTypes WHERE r.isActive = true AND r.embedding IS NOT NULL")
    ...
    /** 배치 점수 산출(공기업): 활성이면서 NCS 임베딩이 생성된 이력서 전체 조회 (Member + careerTypes JOIN FETCH) */
    @Query("SELECT DISTINCT r FROM Resumes r JOIN FETCH r.member m LEFT JOIN FETCH m.careerTypes WHERE r.isActive = true AND r.ncsEmbedding IS NOT NULL")
