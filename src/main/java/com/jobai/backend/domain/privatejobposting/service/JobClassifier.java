...
public class JobClassifier {
    ...
    private static final String REGION_SYSTEM_PROMPT = """
            ...
            - 출력은 JSON 배열만. 설명·번호·기타 텍스트 금지.
            - 예: ["서울", "경기", "미확인"]
            """.formatted(Location.labelsForPrompt());
     ...
     */
    public List<Location> classifyRegions(List<String> locations) {
        if (locations == null || locations.isEmpty()) {
            return List.of();
        }
        List<Location> results = new ArrayList<>(locations.size());
    ...
    private List<Location> classifyRegionBatch(List<String> batch) {
        try {
            String userText = buildRegionUserText(batch);
            String response = anthropicClient.complete(REGION_SYSTEM_PROMPT, userText, 4096);
            List<Location> parsed = parseRegionResponse(response, batch.size());
            if (parsed == null) {
    ...
    private List<Location> parseRegionResponse(String response, int expectedSize) {
        try {
            String json = extractJsonArray(response);
            List<String> labels = objectMapper.readValue(json,
                    objectMapper.getTypeFactory().constructCollectionType(List.class, String.class));
    ...
    private List<Location> regionNullList(int n) {
        return new ArrayList<>(Collections.nCopies(n, null));
    }
}
